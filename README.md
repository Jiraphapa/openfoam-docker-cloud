...
This will create a VM instance with the Container-Optimized OS, pre-installed Docker runtime and packages. You can access the command-line interface of an instance via ssh to the VM's External IP address (adding your public SSH Key is required) or from the GCP console, **VM instances** page by choosing **SSH** > **Open in browser window** under the **Connect** instance tab ([How to connect to your instances?](https://cloud.google.com/compute/docs/instances/connecting-to-instance#console))
